...
class SIZE(LefStatement):
    def __init__(self, point: db.Point):
        self.point=point

    def format(self):
        return " {} BY {}".format(self.point.x, self.point.y)
...
class Port(LefContainer):
    ...
    def __init__(self, geometries: List[Tuple[Layer, List[Union[Rect, Polygon]]]]):
...
class Pin(LefContainer):
    ...
    def __init__(self, pin_name: str, direction: Direction, use: Use, port: Port,
...
class Pin2(Pin):
    """
    terminal pin that have SHAPE
    """
    def __init__(self, pin_name: str, direction: Direction, use: Use, port: Port,
                 property: Dict[str, Any],shape: Shape):
        Pin.__init__(self, pin_name, direction, use, port,property)
        self.shape=shape
